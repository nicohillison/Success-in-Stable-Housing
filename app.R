...
ui <- fluidPage(
          tabsetPanel(
              tabPanel("My Dashboard",
                       fluidRow(h1("Success in Finding Permanent Housing by State")),
                       fluidRow(box(title = "Data Description: ",
                                    width = 10,
                                    solidHeader = TRUE,
                                    "This is a dataset  on system performance measure information by Continuum of Care since FY 2015.  
                                    It was originally used to consistently calculate and analyze system performance measures between 
                                    Continuums of Care (CoCs), Homeless Management Information System (HMIS) administrators, and HMIS 
                                    vendors. We are only using the data from 2019 to be consistent from our poster presentation. 
                                    We got the data from data.world. Here is a link with more information about the dataset: 
                                    https://data.world/mschnars/homeless-system-performance-measures"))
              ),
              tabPanel("Continuum of Care Award Amount ($)",
                       fluidRow(h1("Continuum of Care Award Amount ($)")),
                       sidebarLayout(
                         sidebarPanel(
                           selectInput(
                             inputId = "State",
                             ...
                             choices = list("ALL STATES", "AK", "AL", "AR", "AZ", "CA", "CO", "CT", "DC", "DE", "FL", 
                               ...
                               box(title = "Average Increase in Wage by Days in Shelter: ",
                               width = 15,
                               solidHeader = TRUE,
                               "In this scatter plot, we are looking at the average percent increase 
                           in wage after the average amount of days spent in a shelter. 
                           This is colored by the type of living environment they are currently in. 
                           When hovering by the data, one will also be able to see from what state did the data come from.")
                        ),
                       box(plotlyOutput(outputId = "scatter2Plot"))),
                       sidebarLayout(
                         sidebarPanel(
                           box(title = "Average CoC Award Amount vs. % Return to Shelter: ",
                               width = 15,
                               solidHeader = TRUE,
                               "Olivia's Text.")
                           ),
              ...
              tabPanel("Percent of Exit",
                       fluidRow(h1("Percent of Exits")),
                       sidebarLayout(
                         sidebarPanel(
                           box(title = "Percent Exiting Street Outreach by State: ",
                               width = 15,
                               solidHeader = TRUE,
                               "Olivia's text.")
                         ),
                         ...
                         sidebarLayout(
                           sidebarPanel(
                             box(title = "Percent Exiting by State: ",
                                 width = 15,
                                 solidHeader = TRUE,
                                 "In this bar graph we are looking at the percentage of successful exit from emergency shelter, 
                                 transitional housing, safe haven, and permanent housing-rapid rehousing to permanent housing 
                                 by each state. As seen in the graph, the state of Maine is around the 43% mark. Showing that 
                                 there is 57% of unsuccessful exits in the state.")
                           ),
                       ...
                       sidebarLayout(
                         ...
                         sidebarPanel(
                           ...
                           box(title = "Percent Returning by Program Type: ",
                               width = 15,
                               solidHeader = TRUE,
                               "In the first, interactive graph, we visualize the percent of individuals that left the 
                               shelter within each state by shelter program type. The 5 types of shelter programs are Street Outreach, 
                               Emergency Shelter, Temporary Housing, Safe Haven, and Permanent Housing. Again, when the individual hovers 
                               over each state, they are able to see the specific numeric amount that corresponds to the state’s program return 
                               percentage in 12 months."),
                           box(title = "Percent Returning to Shelter Within 24 Months: ",
                               width = 15,
                               solidHeader = TRUE,
                               "In the second graph, we visualize the return to shelter percentage rates (within 24 months) averaged across all states. 
                               From this visual we find that North and South Dakota have the highest return to shelter rates and Montana and Rhode 
                               Island have the lowest return rates. By hovering over each state, one not only finds the numeric percentage return 
                               to shelter rate but also on average how much Continuum of Care spending is given to the shelters in the state, the 
                               average number of beds available per shelter, and the individual count of people returning to the shelter within 24 months.")
                           ),
